...
class SpatialImagesController(TemplateController):
    ...
    def update_slices(self, image):
        ...
        if x_idx > gray_image.shape[1]:
            x_idx = gray_image.shape[1] - 1
        if y_idx > gray_image.shape[0]:
            y_idx = gray_image.shape[0] - 1
        x_data = gray_image[y_idx-1, :]
        y_data = gray_image[:, x_idx-1]
