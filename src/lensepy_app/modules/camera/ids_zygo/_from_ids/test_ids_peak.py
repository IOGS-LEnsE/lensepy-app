...
def main():
    ...
    try:
        ...
        remote_nodemap.FindNode("AcquisitionFrameRate").SetValue(10)
        remote_nodemap.FindNode("ExposureTime").SetValue(10000)
        ...
        numb_img = 5
        print(f"Getting {numb_img} images...")
        # Process numb_img images
        for _ in range(numb_img):
            try:
                # Wait for finished/filled buffer event
                buffer = data_stream.WaitForFinishedBuffer(100000)
                img = ids_peak_ipl_extension.BufferToImage(buffer)
                data = img.get_numpy_2D().copy()

                # Do something with `img` here ...
                resized_down = cv2.resize(data, (800, 600), interpolation=cv2.INTER_LINEAR)

                cv2.imshow("IDS Camera", resized_down)
